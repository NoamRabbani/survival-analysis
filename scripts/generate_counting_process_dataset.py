...
def main():
    ...
    # rows = cp.process_issue("issues_hbase/HBASE-8375",
    #                         input_paths, reputations)
...
class CountingProcess:
    ...
    def generate_dataset(self, input_paths, output_paths, reputations,
        ...
        for filename in os.listdir(input_paths["issues"]):
            ...
            issue_dates, issue_states = self.generate_issue_states(
                issue_path, input_paths, reputations, workloads)
    ...
    def add_assignee_workload_feature(self, issue, issue_states, issue_dates,
        ...
        # do a pass to set the workload of unassigned issues to -1
        for date in issue_dates:
            if issue_states[date]["assignee"] == "unassigned":
                issue_states[date]["assignee_workload"] = -1
