...
# issues$is_assigned.time <- issues$link_count:issues$start
...
feature_set <- "full"
if (feature_set == "full") {
    # f <- coxph(S ~ priority + issuetype + is_assigned + comment_count + link_count +
    #      affect_count + fix_count + has_priority_change + 
    #      has_desc_change + has_fix_change + reporter_rep + assignee_workload,
    #      x=TRUE, y=TRUE)
    f <- coxph(S ~ priority + issuetype + is_assigned + rcs(comment_count,4) + rcs(link_count, 4) +
    ...
    f <- coxph(S ~ priority + issuetype + is_assigned + comment_count + rcs(link_count, 4) +
         rcs(affect_count, 4) + rcs(fix_count,4) + has_priority_change + 
         has_desc_change + rcs(has_fix_change,4) + rcs(reporter_rep,4) + rcs(assignee_workload,4),
         x=TRUE, y=TRUE)
}
...
f.short <- coxph(S ~ z, x=TRUE, y=TRUE)
